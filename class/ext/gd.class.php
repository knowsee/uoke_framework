...
class ext_gd {
    ...
    private function gd_fit() {
        ...
        switch ($otype) {
            case 1: $source_img = imagecreatefromgif($this->_source);
                break;
            case 2: $source_img = imagecreatefromjpeg($this->_source);
                break;
            case 3: $source_img = imagecreatefrompng($this->_source);
                break;
        ...
        // 生成图片  
        switch ($dest_ext) {
            case 1: imagegif($new_img, $this->_dest, $this->_quality);
                break;
            case 2: imagejpeg($new_img, $this->_dest, $this->_quality);
                break;
            case 3: imagepng($new_img, $this->_dest, (int) (($this->_quality - 1) / 10));
                break;
        }
        ...
        if ($this->_watermark != null && file_exists($this->_watermark) && file_exists($dest)) {
            ...
            if ($w <= $owidth && $h <= $oheight) {
                ...
                if ($this->_handler == 'imagemagick') { // imagemagick 添加水印  
                    ...
                    switch ($wtype) {
                        case 1: $water_img = imagecreatefromgif($this->_watermark);
                            break;
                        case 2: $water_img = imagecreatefromjpeg($this->_watermark);
                            break;
                        case 3: $water_img = imagecreatefrompng($this->_watermark);
                            break;
                        default: return false;
                    }

                    switch ($otype) {
                        case 1: $dest_img = imagecreatefromgif($dest);
                            break;
                        case 2: $dest_img = imagecreatefromjpeg($dest);
                            break;
                        case 3: $dest_img = imagecreatefrompng($dest);
                            break;
                        default: return false;
                    }
                    ...
                    switch ($otype) {
                        case 1:imagegif($dest_img, $dest, $this->_quality);
                            break;
                        case 2:imagejpeg($dest_img, $dest, $this->_quality);
                            break;
                        case 3:imagepng($dest_img, $dest, (int) (($this->_quality - 1) / 10));
                            break;
                    }
    ...
    private function to_log($msg) {
        log::writeLog($msg, 'images_class', log::IMPORTANT);
    }
